...
def clone_input_dataset(input_h5_path: os.path,
                        output_h5_path: os.path,
                        data_key: str):
    with h5py.File(input_h5_path, 'r') as input_h5_file:
        lr_images_dataset = input_h5_file[data_key]

        num_images = lr_images_dataset.shape[0]
        num_views = lr_images_dataset.shape[1]
        num_channels = lr_images_dataset.shape[2]
        height = lr_images_dataset.shape[3]
        width = lr_images_dataset.shape[4]
        ...
        with h5py.File(output_h5_path, 'w') as output_h5_file:
            new_dataset = output_h5_file.create_dataset(
                name=data_key,
                shape=new_dataset_shape,
                dtype=lr_images_dataset.dtype,
                compression='lzf',
            ...
            print("Cloning light field dataset.")
            bar = progressbar.ProgressBar()
            for idx in bar(np.arange(num_images)):
                new_dataset[idx] = lr_images_dataset[idx]


def create_new_datasets(input_h5_path: os.path,
                        output_h5_path: os.path,
                        data_key: str):
    clone_input_dataset(input_h5_path, output_h5_path, data_key)

    downsample_dataset(output_h5_path, data_key)
    ...
    extract_indexed_dataset(output_h5_path, data_key_hr, x_indices, '_x')
    extract_indexed_dataset(output_h5_path, data_key_hr, y_indices, '_y')

    extract_indexed_dataset(output_h5_path, data_key_lr, x_indices, '_x')
    extract_indexed_dataset(output_h5_path, data_key_lr, y_indices, '_y')
...
if __name__ == '__main__':
    ...
    try:
        create_new_datasets(input_path, output_path, args.key)
