#!/usr/bin/env python3
"""
Copyright Seán Bruton, Trinity College Dublin, 2017. 
Contact sbruton[á]tcd.ie.
"""
from torch import optim

from .experiment import Experiment
from ..datasets.light_field_dataset import LightFieldDataset
from ..models import LFVolBaseNet


class LFBaseNetExperiment(Experiment):
    def __init__(self, config):
        super().__init__(config=config)

        self.train_set = LightFieldDataset(
            dataset_loc=config['dataset_loc'],
            training=True
        )

        self.test_set = LightFieldDataset(
            dataset_loc=config['dataset_loc'],
            training=False
        )

        self.model = LFVolBaseNet()

        self.optimizer = optim.Adamax(
            params=self.model.parameters()
        )
